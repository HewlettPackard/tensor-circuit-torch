...
from .tensor_circuit import NonlinearPhotonicCircuit
...
class MPSCircuitOptimizer(ABC):
    ...
    def epoch_train(
        ...
        # optimizer
        optimizer = optim.Adam(self.circuit.get_params(), lr=lr)
        ...
        for epoch in range(max_epochs):
            ...
            if param_lims is not None:
                for p in self.circuit.get_params():
            ...
            if verbose and (epoch + 1) % epoch_print == 0:  
                print(f'\nEpoch {epoch + 1} in {time() - epochs_start:.2f}s, objective: {score}')
                epochs_start = time()
    ...
    def LBFGS_train(
        ...
        # init optimizer
        optimizer = optim.LBFGS(self.circuit.get_params(), lr=lr, **solver_args)
        ...
        for step in range(num_steps):
            ...
            if track_params:
                params_list.append(self.circuit.get_params())

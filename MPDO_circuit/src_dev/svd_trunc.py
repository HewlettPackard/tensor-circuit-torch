...
def svd_trunc(A: torch.Tensor, 
    ...
    U_trunc, s_trunc, Vd_trunc, normalize_factor = SVDTrunc.apply(
        A + reg, cutoff, max_num, normalize, lowrank, num_select)
...
class SVDTrunc(Function):
    ...
    def forward(ctx, A: torch.Tensor, cutoff: float=1e-8, max_num: int|None=None, 
                normalize: bool=True, lowrank: bool=True, num_select: int|None=None
                ) -> Tuple[torch.Tensor]:
        """
        ...
        n_select: int|None (default None)
            Select n_select SVDs randomly with probabilities s^2.
        ...
        # Compute the truncated SV dimension, from cutoff and max_num if no n_select given
        if num_select is None:
            bdim = torch.maximum(
                torch.minimum(
                    ((s ** 2 / (s ** 2).sum()) > cutoff).sum(axis=-1).max(), torch.tensor(max_num, device=A.device)
                    ), 
            ...
            probs = s.pow(2) / s.pow(2).sum() # get probabilities from normalized s^2
            print(f"ps: {probs}")
            samples = torch.multinomial(probs, np.minimum(num_select, max_num), replacement=False).sort()[0]
            U_trunc, s_trunc, Vd_trunc = U[..., :, samples], s[..., samples], Vd[..., samples, :]
